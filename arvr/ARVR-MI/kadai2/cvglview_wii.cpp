...
template <typename T = float> T clamp(T v, T min, T max) {
  if (v < min)
    return min;
  if (v > max)
    return max;
...
void drawTeapot() {
  ...
  std::cout << wpitch << ", " << wroll << std::endl;
  glTranslatef(pos_wii.x, pos_wii.y, pos_wii.z);

  /* Wiiリモコンの傾きを色にマッピングするように変更する */
  const double r = clamp<double>(0.5 + wroll, 0, 1);
  const double g = clamp<double>(0.5 + wpitch, 0, 1);
  glColor3d(r, g, 1.0);
  /* 課題 */
...
void display(void) {
  ...
  glTranslatef(pos_wii.x, pos_wii.y, pos_wii.z);
  glTranslatef(transGL[0], transGL[1], transGL[2]);
