...
void process(IplImage *source, IplImage *destination) {
  ...
  for (row = 0; row < bankSize; row++) {
    ...
    for (col = 0; col < bankSize; col++) {
      double m = row - offset;
      double n = col - offset;
      const double sq_s2 = 2.0 * sigma * sigma;
      const double m2n2 = m * m + n * n;
      const double pi = 3.1415926; // std::numbers::pi;
      filter[row][col] = (m2n2 - sq_s2) * std::exp(-m2n2 / sq_s2);
       // /(2.0 * pi * std::pow(sigma, 6));
  ...
  for (row = 0; row < bankSize; row++) {
    for (col = 0; col < bankSize; col++) {
      std::cout << filter[row][col] << std::endl;
    }
  }
  /* exit(1); */
    ...
    for (x = size.width / 4; x < (size.width * 3 / 4); x++) {
      ...
      for (j = 0; j < bankSize; j++) {
        for (i = 0; i < bankSize; i++) {
          ...
          double intensityS =
              (0.114 * blueS + 0.587 * greenS + 0.299 * redS) / 255.0;
