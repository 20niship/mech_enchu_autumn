...
void search_template(const CvArr *tmp, const CvArr *curr,
                     const CvPoint2D32f template_feature,
                     CvPoint2D32f *curr_feature_p, CvSize size, int step,
                     char *status_p);
  ...
  for (;;) {
    ...
    if (mode == SEARCHING) {
      ...
      cvRectangle(template_gray, cvPoint(p1x - 1, p1y - 1),
                  cvPoint(p2x + 1, p2y + 1), CV_RGB(0, 255, 0),
                  1, // thickness, 1
                  8, // line_type, 8=8-connected
                  0);
    ...
    if ((mode == SELECTING_SECOND_CORNER) & (selection.width > 0) &&
        (selection.height > 0)) {
      cvRectangle(output, cvPoint(selection.x, selection.y),
    ...
    cvLine(output, cvPoint(mouse_position.x, 0),
           cvPoint(mouse_position.x, output->height - 1), CV_RGB(0, 255, 255),
           1, 8, 0);
    cvLine(output, cvPoint(0, mouse_position.y),
           cvPoint(output->width - 1, mouse_position.y), CV_RGB(0, 255, 255), 1,
           8, 0);
