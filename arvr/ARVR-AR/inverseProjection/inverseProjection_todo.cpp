...
void calibrate(CvMat *cornerPoints, CvMat *objectPoints, CvMat *intrinsic,
               CvMat *distortion, CvMat *rotation, CvMat *translation);

void drawPlaneImage(IplImage *output, IplImage *plane, CvMat *intrinsic,
                    CvMat *rotation, CvMat *translation, ChessBoard board);

int regionCheck(CvPoint2D32f point, ChessBoard board);

void computePositionOnPlane(CvPoint m, CvMat *intrinsic, CvMat *rotation,
                            CvMat *translation, CvPoint2D32f *result);
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if (cvFindChessboardCorners(gray, chess.patternSize, points,
      ...
      cvFindCornerSubPix(gray, points, detectedPointsNum, cvSize(5, 5),
                         cvSize(-1, -1),
    ...
    cvDrawChessboardCorners(src2, chess.patternSize, points, detectedPointsNum,
                            allPointsFound);
    ...
    if (allPointsFound) {
      ...
      calibrate(cornerPoints, objectPoints, intrinsic, distortion, rotation,
                translation);
      ...
      */
      ...
      drawPlaneImage(output, pyonkichiImage, intrinsic, rotation, translation,
                     chess);
    }
...
void calibrate(CvMat *cornerPoints, CvMat *objectPoints, CvMat *intrinsic,
  ...
  cvFindExtrinsicCameraParams2(objectPoints, cornerPoints, intrinsic,
                               distortion, rotVector, translation);
  ...
  */
  ...
  for (i = 0; i < 3; i++) {
    cvmSet(translation, i, 0,
           cvmGet(translation, i, 0) + cvmGet(rotation, i, 0) * xOffset);
  }

  for (i = 0; i < 3; i++) {
    cvmSet(rotation, i, 0, -cvmGet(rotation, i, 0));
    cvmSet(rotation, i, 2, -cvmGet(rotation, i, 2));
  }
...
void drawPlaneImage(IplImage *output, IplImage *plane, CvMat *intrinsic,
  ...
  for (u = 0; u < width; u++) {
    for (v = 0; v < height; v++) {
      ...
      computePositionOnPlane(cvPoint(u, v), intrinsic, rotation, translation,
                             &position);
...
int regionCheck(CvPoint2D32f point, ChessBoard board) {
  float x = point.x;
  float y = point.y;
  if ((x >= 0) && (x < board.dx * (board.patternSize.width - 1)) && (y >= 0) &&
