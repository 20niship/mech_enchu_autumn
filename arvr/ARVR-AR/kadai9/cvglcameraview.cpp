...
void drawCameraTexture(double t[3], double r[4]) {
  ...
  /* 課題 */
  /* 適切なパラメータへ変更する */
  glRotated(0.0 * 90.0, 0.0, 1.0, 0.0);
  /* 課題 */
  /* 適切なパラメータへ変更する */
  glRotated(0.0 * 180.0, 0.0, 0.0, 1.0);
#else
  glTranslated(t[0], t[1], t[2]);
  /* glRotated(r[0], r[1], r[2], r[3]); */
  glRotated(r[1] * 90.0, 0.0, 1.0, 0.0);
...
void *captureThread(void *args) {
  ...
  frame_copy = cvCreateImage(cvGetSize(frame), frame->depth, 3);
...
void loadIntrinsicParams(const char *fileName, CvMat *intrinsic,
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      double value = 0;
      const auto unused = fscanf(file, "%lf", &value);
...
void calibrate(CvMat *cornerPoints, CvMat *objectPoints, CvMat *intrinsic,
  ...
  cvFindExtrinsicCameraParams2(objectPoints, cornerPoints, intrinsic,
                               distortion, rotVector, translation, 0);
...
void updateTexture(IplImage *image) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, resized_frame->width,
               resized_frame->height, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE,
