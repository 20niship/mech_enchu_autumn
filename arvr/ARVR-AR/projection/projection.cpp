...
int main(int argc, char *argv[]) {
  ...
  const char *intCalibName = "../param/intrinsic_param_ref.txt";
  const char *extCalibName = "../param/extrinsic_param.txt";
  const char *boardFileName = "../param/board.jpg";

  const char *windowName = "projection";
  ...
  for (i = 0; i < PROJECTED_POINTS; i++) {
    ...
    project(&objectPoints[i], &projectedPoints[i], intrinsic, rotation,
            translation);
  }
  ...
  cvCircle(output, cvPointFrom32f(projectedPoints[4]), 5, CV_RGB(255, 0, 0), 2,
           8, 0);

  cvCircle(output, cvPointFrom32f(projectedPoints[5]), 5, CV_RGB(255, 0, 0), 2,
           8, 0);

  cvPutText(output, "O", cvPointFrom32f(projectedPoints[0]), &font,
            CV_RGB(255, 0, 0));
  cvPutText(output, "x", cvPointFrom32f(projectedPoints[1]), &font, xAxisColor);
  cvPutText(output, "y", cvPointFrom32f(projectedPoints[2]), &font, yAxisColor);
  cvPutText(output, "z", cvPointFrom32f(projectedPoints[3]), &font, zAxisColor);

  // modify character info. in image.
  cvPutText(output, "(40 mm, 60 mm)", cvPointFrom32f(projectedPoints[4]), &font,
            CV_RGB(255, 0, 0));

  cvPutText(output, "(80 mm, 120mm)", cvPointFrom32f(projectedPoints[5]),
            &font, CV_RGB(255, 0, 0));
...
void project(CvPoint3D32f *objectPoint, CvPoint2D32f *projectedPoint,
  ...
  /* tmp[2] = ...; */
  for (int i = 0; i < 3; i++) {
    tmp[i] = R[i][0] * x[0] + R[i][1] * x[1] + R[i][2] * x[2] + o[i];
  }
  ...
  /* projected[2] = ...; */
  for (int i = 0; i < 3; i++) {
    projected[i] = A[i][0] * tmp[0] + A[i][1] * tmp[1] + A[i][2] * tmp[2];
  }
