...
void saveExtrinsicParams(const char *fileName, CvMat *rotation,
                         CvMat *translation);

void calibrateCamera(const char *intrinsicFileName,
                     const char *extrinsicFileName, CvMat *cornerPoints,
                     CvMat *objectPoints);
...
int main(int argc, char **argv) {
  ...
  char key;
  int camID;
  const char *windowName = "extrinsic calibration";
  ...
  while (1) {
    ...
    if (key == RETURN && allPointsFound) {
      ...
      calibrateCamera("../param/intrinsic_param_ref.txt",
                      "../param/extrinsic_param.txt", cornerPoints,
                      objectPoints);
...
void saveExtrinsicParams(const char *fileName, CvMat *rotation,
  ...
  fprintf(file, "%lf %lf %lf\n", -cvmGet(R, 0, 0), cvmGet(R, 0, 1),
          -cvmGet(R, 0, 2));
  fprintf(file, "%lf %lf %lf\n", -cvmGet(R, 1, 0), cvmGet(R, 1, 1),
          -cvmGet(R, 1, 2));
  fprintf(file, "%lf %lf %lf\n", -cvmGet(R, 2, 0), cvmGet(R, 2, 1),
          -cvmGet(R, 2, 2));
