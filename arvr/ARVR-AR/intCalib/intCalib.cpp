...
void store2DCoordinates(vector<vector<Point2f>> *cornerPoints,
                        vector<Point2f> points, ChessBoard chess, int imageID);
void store3DCoordinates(vector<vector<Point3f>> *objectPoints, ChessBoard chess,
                        int imageID);
...
int main(int argc, char **argv) {
  ...
  int i, j;
  int camID;
  const char *windowName = "intrinsic calibration";
  ...
  gray = Mat(cvSize(smallSrc.rows, smallSrc.cols), smallSrc.dims, 1);
  ...
  while (1) {
    ...
    if (findChessboardCorners(gray, chess.patternSize, points,
    ...
    if (key == RETURN && allPointsFound) {
      ...
      if (currImgNum == IMAGE_NUM) {
        calibrateCameraIntrinsic("../param/intrinsic_param_03220283.txt", chess,
...
void saveIntrinsicParams(const char *fileName, Mat intrinsic, Mat distortion) {
  FILE *file;
...
void calibrateCameraIntrinsic(const char *intrinsicFileName, ChessBoard chess,
  ...
  calibrateCamera(objectPoints, cornerPoints, chess.patternSize, intrinsic,
