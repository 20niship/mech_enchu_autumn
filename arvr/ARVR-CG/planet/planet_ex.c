...
void display(void) {
  ...
  glPushMatrix();
  {
    ...
    glPushMatrix();
    {
      // --- 課題 ---
      // 太陽の自転の計算式
      glRotatef(rot_vel_sun * day, 0.0, 1.0, 0.0);
    ...
    glPushMatrix();
    {
      ...
      glRotatef(day * rot_vel_venus, 0.0, 1.0, 0.0);
    ...
    glPushMatrix();
    {
      // --- 課題 ---
      // 地球の公転の計算式
      glRotatef(rev_vel_earth * day, 0.0, 1.0, 0.0);
      glTranslatef(orbit_radius_earth, 0.0, 0.0);

      // --- 課題 ---
      // 地球の自転の計算式
      glRotatef(rot_vel_earth * day, 0.0, 1.0, 0.0);
      ...
      {
        // ---[edit]
        glRotatef(180 + rev_vel_moon * day, 0.0, 1.0, 0.0);
        glTranslatef(orbit_radius_moon, 0.0, 0.0);
        glRotatef(day * rot_vel_moon, 0.0, 1.0, 0.0);
        glMaterialfv(GL_FRONT, GL_AMBIENT, mat_color_moon);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, mat_color_moon);
    ...
    glPushMatrix();
    {
      glRotatef(rev_vel_mars * day, 0.0, 1.0, 0.0);
