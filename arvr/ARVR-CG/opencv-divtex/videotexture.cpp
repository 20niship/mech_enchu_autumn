...
#define DEG (2 * PI / 360.)
#define FOV_Y 45 /* カメラの画角 */
#define NEAR_CLIP 10.0
#define FAR_CLIP 10000.0
#define DISTANCE_0 579.41
#define DIV_X 64 /* x方向のテクスチャ分割数 */
#define DIV_Y 48 /* y方向のテクスチャ分割数 */
...
GLfloat tex_coord_v[DIV_Y + 1][DIV_X + 1][2]; /* テクスチャ座標行列 */
GLfloat vertex_v[DIV_Y + 1][DIV_X + 1][3];    /* ポリゴン頂点行列 */
...
void define_texturesize(GLsizei *w0, GLsizei *h0, GLsizei *w1, GLsizei *h1) {
  ...
  while (*w0 > (prev = (int)pow(2, i)))
    i++;
  *w1 = prev;
  i = 0;
  while (*h0 > (prev = (int)pow(2, i)))
    i++;
  *h1 = prev;
}
...
void generate_surface(void) {
  int i, j;
  /* 課題：ここのテクスチャ座標の指定を修正せよ */
  /* TODO_s と TODO_e の間を変更する */
  for (j = 0; j <= DIV_Y; j++) {
    for (i = 0; i <= DIV_X; i++) {
      vertex_v[j][i][0] = ((float)i / (float)DIV_X - 0.5) * (float)win_width;
      vertex_v[j][i][1] = ((float)j / (float)DIV_Y - 0.5) * (float)win_height;
      vertex_v[j][i][2] = 0.0;
      const int di = i - DIV_X / 2;
      const int dj = j - DIV_Y / 2;
      const float d = (float)(di * di + dj * dj) / 50;
      const float ratio = 2.0;
      tex_coord_v[j][i][0] = (float)(i + ratio * std::sin(d)) / (float)DIV_X *
                             (float)cap_width / (float)tex_width;
      tex_coord_v[j][i][1] = (float)(j + ratio * std::cos(d)) / (float)DIV_Y *
                             (float)cap_height / (float)tex_height;
    }
  }
}
...
void motion(int x, int y) {
  ...
  case GLUT_RIGHT_BUTTON:
    if (y == mouse_y)
      return;
    if (y < mouse_y)
      distance += (GLfloat)(mouse_y - y);
    else
      distance -= (GLfloat)(y - mouse_y);
...
void idle(void) {
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame->width, frame->height, GL_RGB,
                  GL_UNSIGNED_BYTE, frame->imageData); /* 画像の更新 */
  glutPostRedisplay();
}
...
void my_initGL(void) {
  ...
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, cap_width, cap_height, GL_RGB,
