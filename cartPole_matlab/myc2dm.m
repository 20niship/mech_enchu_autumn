function [adout,bd,cd,dd] = myc2dm(a,b,c,d,Ts)
%%% Conversion of continuous LTI systems to discrete-time.
%%% [Ad,Bd,Cd,Dd] = C2DM(A,B,C,D,Ts,'method') 
%%% converts the continuous time state-space system (A,B,C,D) to discrete time using 'method':
...
%%% converts the continuous time polynomial transfer function G(s) = NUM(s)/DEN(s)
%%% to discrete time, G(z) = NUMd(z)/DENd(z), using 'method'.
%%% See also: MYC2D.
%%%
%%% Date : 2011/10/05
